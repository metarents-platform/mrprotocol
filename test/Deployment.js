...
describe("Test on deployment of Gateway & RNFT on the blockchain", async () => {
    ...
    beforeEach(async () => {
        ...
        gateway = await upgrades.deployProxy(Gateway, [rNFT.address, treasury.address], {initializer: "initialize"});
    ...
    describe("Deployment of Gateway", function () {
        ...
        it("The deployer should have the admin role, others not yet", async function () {
            ...
            expect(await gateway.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.equal(true);
            expect(await gateway.hasRole(DEFAULT_ADMIN_ROLE, hacker.address)).to.equal(false);
        });
        ...
        it("USDC should be listed as the supported token", async () => {
            ...
            expect (await gateway.isSupportedPaymentToken(USDC_ADDRESS)).to.equal(true);
        });
        ...
        it("Other tokens beyod MANA should not be listed as the supported token", async () => {
            ...
            expect (await gateway.isSupportedPaymentToken(RAND_TOKEN_ADDRESS)).to.equal(false);
        });
